...
export const ProductionCharts = ({ employees, orders }: ProductionChartsProps) => {
  ...
  const pieData = useMemo(() => {
    if (!orders || orders.length === 0) return [{ name: "لا توجد طلبات", value: 1, label: "لا توجد طلبات" }];
    
    return orders
      .filter(order => order.product && order.client) // Make sure order has product and client
      .map(order => ({
        name: order.client,
  ...
  const barData = useMemo(() => {
    if (!employees || employees.length === 0) return [{ name: "لا يوجد عمال", production: 0, target: 0 }];
    
    return employees
      .filter(employee => employee.name) // Make sure employee has name
      .map(employee => ({
        name: employee.name,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <div className="bg-white p-4 rounded-md shadow-sm">
        ...
        {employees.length === 0 ? noDataDisplay : (
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart 
                data={barData}
